...
COPY chrome-driver/chromedriver.exe /bin/chrome-driver/chromedriver.exe
COPY Manifest.txt bin


WORKDIR bin
RUN jar -cvfm Program.jar Manifest.txt ./pkt/*.class
CMD ["java", "-cp", ".:selenium-server/*:Program.jar", "-Dwebdriver.chrome.driver=chromedriver", "pkt.Program"]
